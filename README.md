# Веб-приложение для получения отчетов из подключенных баз данных (Raporterra)

Вместе с командой разрабатывал WEB-приложение для получения отчетов из подключенных баз данных (Raporterra).

Сервис позволяет для подключения к внешним базам данных и формирования отчётов на основе SQL-запросов.  
Приложение позволяет пользователям просматривать и выгружать отчёты в формате .xlsx, а администраторам - управлять подключениями и настройками системы.

## В рамках проекта:
- Реализовал взаимодействие базы данных с приложением через Hibernate.
